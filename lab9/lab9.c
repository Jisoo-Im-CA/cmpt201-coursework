...
port number).
- IP address = 127.0.0.1 on port 8000

2. Is it UDP or TCP? How do you know?
- It is TCP because the socket is creadted with SOCK_STREAM

3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code?
- The data comes from standard input. read(STDIN_FILENO, buf, BUF_SIZE) means it
reads from fd

4. How does the client program end? How can you tell that in the code?
- The client ends when read() returns 0 or 1, exiting the while-loop and calling
close(sfd).
