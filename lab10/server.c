...
- MAX 4 clients: client uses a TCP socket and sends data via write()
2. What data is the client sending to the server?
- send 5 fixed strings : Hello, Apple, Car, Green, Dog
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
- the argument receives a struct containing a run flag, the shared list, and its
mutex

2. How are received messages stored?
- sotred in a linked list

3. What does `main()` do with the received messages?
- wait for all msgs, stop threads, collect and frees stored msgs.

4. How are threads used in this sample code?
- acceptor thread handles connections
*/

/*
Non-blocking Sockets:
- Sockets are made non-blocking using fcntl(fd, F_SETFL, flags | O_NONBLOCK).
- Both the server's listening socket and each client connection socket are made
non-blocking.
- This allows accept() and read() to return immediately instead of blocking,
letting threads run independently without getting stuck.
*/
...
int main() {
  ...
  while (1) {
    ...
    uint32_t cnt = list_handle.count;
    pthread_mutex_unlock(&list_mutex);

    if (cnt >= MAX_CLIENTS * NUM_MSG_PER_CLIENT)
      break;
