...
#define MAX_FILE_SIZE 512
  ...
  FILE *mf = fopen(message_path, "rb");
  if (!mf) {
    perror("fopen");
    return -1;
  }
  message_len = fread(message, 1, MAX_FILE_SIZE, mf);
  fclose(mf);

  FILE *sf = fopen(sign_path, "rb");
  if (!sf) {
    perror("fopen_sig");
    return -1;
  }
  sig_len = fread(signature, 1, MAX_FILE_SIZE, sf);
  fclose(sf);
  ...
  int rc = EVP_DigestVerifyFinal(ctx, signature, sig_len);
